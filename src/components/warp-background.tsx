...
export const WarpBackground: React.FC<WarpBackgroundProps> = ({
  ...
  const [topBeams, setTopBeams] = useState<BeamProps[]>([]);
  const [rightBeams, setRightBeams] = useState<BeamProps[]>([]);
  const [bottomBeams, setBottomBeams] = useState<BeamProps[]>([]);
  const [leftBeams, setLeftBeams] = useState<BeamProps[]>([]);
  ...
  return (
    <div className={cn("relative rounded p-20", className)} {...props}>
      ...
      >
        {/* 上面 */}
        <div className="absolute [transform-style:preserve-3d] [background-size:var(--beam-size)_var(--beam-size)] [background:linear-gradient(var(--grid-color)_0_1px,_transparent_1px_var(--beam-size))_50%_-0.5px_/var(--beam-size)_var(--beam-size),linear-gradient(90deg,_var(--grid-color)_0_1px,_transparent_1px_var(--beam-size))_50%_50%_/var(--beam-size)_var(--beam-size)] [container-type:inline-size] h-full [transform-origin:50%_0%] [transform:rotateX(-90deg)] w-full">
        ...
        {/* 左側 */}
        <div className="absolute left-0 top-0 [transform-style:preserve-3d] [background-size:var(--beam-size)_var(--beam-size)] [background:linear-gradient(var(--grid-color)_0_1px,_transparent_1px_var(--beam-size))_50%_-0.5px_/var(--beam-size)_var(--beam-size),linear-gradient(90deg,_var(--grid-color)_0_1px,_transparent_1px_var(--beam-size))_50%_50%_/var(--beam-size)_var(--beam-size)] [container-type:inline-size] h-full [transform-origin:0%_0%] [transform:rotate(90deg)_rotateX(-90deg)] w-full">
          {leftBeams.map((beam, index) => (
            <Beam key={`left-${index}`} {...beam} />
          ))}
        </div>
        {/* 右側 */}
        <div className="absolute right-0 top-0 [transform-style:preserve-3d] [background-size:var(--beam-size)_var(--beam-size)] [background:linear-gradient(var(--grid-color)_0_1px,_transparent_1px_var(--beam-size))_50%_-0.5px_/var(--beam-size)_var(--beam-size),linear-gradient(90deg,_var(--grid-color)_0_1px,_transparent_1px_var(--beam-size))_50%_50%_/var(--beam-size)_var(--beam-size)] [container-type:inline-size] h-full w-full [transform-origin:100%_0%] [transform:rotate(-90deg)_rotateX(-90deg)]">
